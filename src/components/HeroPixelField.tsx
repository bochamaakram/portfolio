...
/**
 * The page mark as a 12x8 bitmap, out of the home ASCII art. A click stamps
 * this glyph onto the field grid, growing to a neat mark and dissolving
 * through the dither as it fades. Tempo and final size jitter a little per
 * click so no two stamps are quite twins, while every stamp is the same mark.
 */
const LOGO_WIDTH = 12
const LOGO_HEIGHT = 8
const LOGO_ROWS = [
...
/** A live stamp once it tops its charge; its cell size, capped so a wide
 *  wordmark stamp still blooms to a neat banner instead of the whole hero. */
const STAMP_CELL_CAP = 1.6
    ...
    const draw = (time: number) => {
      ...
      if (pings.length > 0) {
        ...
        for (const ping of pings) {
          ...
          stamps.push({
            ...
            y: ping.y,
            cellPx: Math.min(
              wmCW * (ping.from + (ping.to - ping.from) * grow),
              wmCW * STAMP_CELL_CAP,
            ),
            amp: (1 - age) ** 1.7,
          })
      ...
      const stampAt = (cx: number, cy: number) => {
        ...
        for (const stamp of stamps) {
          const lx = Math.floor((cx - stamp.x) / stamp.cellPx + LOGO_WIDTH / 2)
          const ly = Math.floor((cy - stamp.y) / stamp.cellPx + LOGO_HEIGHT / 2)
          if (lx < 0 || ly < 0 || lx >= LOGO_WIDTH || ly >= LOGO_HEIGHT)
            continue
