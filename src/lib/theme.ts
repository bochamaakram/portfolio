...
/** The page mark as an SVG, tinted with the given colour. */
export function markFavicon(fill: string) {
  let rects = ''
  for (let y = 0; y < MARK_HEIGHT; y++)
    for (let x = 0; x < MARK_WIDTH; x++)
      if (MARK_ROWS[y][x] === '1')
        rects += `<rect x="${x}" y="${y}" width="1" height="1"/>`
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${MARK_WIDTH} ${MARK_HEIGHT}"><g fill="${fill}">${rects}</g></svg>`
}
